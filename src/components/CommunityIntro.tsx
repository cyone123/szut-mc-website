...
export const CommunityIntro: React.FC<CommunityIntroProps> = ({ onOpenJoin }) => {
  const divisions = [
    {
      ...
      name: '校园建筑复刻组',
      tag: 'ARCHITECTURE',
      tagColor: 'text-amber-700 dark:text-amber-400 border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40',
    ...
    },
    {
      ...
      name: '红石生电工程组',
      tag: 'AUTOMATION',
      tagColor: 'text-rose-700 dark:text-rose-400 border-rose-300 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/40',
    ...
    },
    {
      ...
      name: '纯净生存开荒组',
      tag: 'SURVIVAL SMP',
      tagColor: 'text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40',
    ...
    },
    {
      ...
      name: '技术运维与研发组',
      tag: 'DEVOPS & TECH',
      tagColor: 'text-cyan-700 dark:text-cyan-400 border-cyan-300 dark:border-cyan-800 bg-cyan-50 dark:bg-cyan-950/40',
  ...
  return (
    <section id="community" className="py-20 relative bg-slate-50/50 dark:bg-[#0b0e14]/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {divisions.map((div) => {
            ...
            return (
              <div
                key={div.id}
                className={`bg-white dark:bg-[#121622] border-2 border-slate-200 dark:border-slate-800 p-6 mc-border transition-all duration-300 group hover:-translate-y-1 ${div.accentColor}`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="p-3 bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-cyan-700 dark:text-cyan-400 group-hover:scale-110 transition-transform">
                ...
                <div className="space-y-1.5 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                  {div.highlights.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300">
                      <span className="w-1.5 h-1.5 bg-cyan-500 rounded-none shrink-0" />
                      <span>{item}</span>
                    </div>
                  ))}
                </div>
        ...
        <div className="bg-white dark:bg-[#141926] border-2 border-slate-300 dark:border-slate-700 p-8 mc-border shadow-lg">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 text-center divide-y lg:divide-y-0 lg:divide-x divide-slate-200 dark:divide-slate-800">
            ...
            <div className="pt-4 lg:pt-0">
              <div className="font-pixel text-2xl sm:text-3xl text-emerald-600 dark:text-emerald-400 mb-1">100%</div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <img
                ...
                className="w-8 h-8 rounded-full bg-white p-0.5 border border-slate-300 dark:border-slate-600 shadow-sm"
              />
