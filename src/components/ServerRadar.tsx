  ...
  return (
    <section id="server-radar" className="py-20 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Main Radar Card */}
        <div className="max-w-4xl mx-auto bg-[#11151f] border-2 border-[#2b3345] shadow-2xl mc-border overflow-hidden">
          
          {/* Top Bar of Server Console */}
          <div className="bg-[#181f2c] px-4 py-3 border-b border-slate-700/80 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <div className="relative flex items-center justify-center">
                <span className={`w-3 h-3 rounded-none ${data?.online ? 'bg-emerald-400 shadow-[0_0_10px_#10b981]' : 'bg-rose-500'}`} />
                {data?.online && (
                  <span className="absolute w-3 h-3 bg-emerald-400 animate-ping opacity-75 rounded-none" />
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={loading}
                title="重新检测服务器状态"
                className="mc-button text-xs py-1.5 px-3 flex items-center gap-1.5 bg-[#252c3b]"
              >
          ...
          <div className="p-6 space-y-6">
            ...
            {/* MOTD Display Box (Minecraft Style) */}
            <div className="bg-black/90 border-2 border-slate-700 p-4 relative shadow-inner">
              <div className="text-[10px] text-slate-500 font-mono mb-1.5 flex items-center justify-between">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              {/* Latency Metric */}
              <div className="bg-[#171c26] border border-slate-700/80 p-3.5 flex items-center gap-3">
                <div className="p-2.5 bg-emerald-950/60 border border-emerald-800 text-emerald-400">
                  <Wifi className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-[11px] text-slate-400 font-mono">网络响应延迟</div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-lg font-bold font-code text-white">
                      {data?.latency ?? 25}
                    </span>
                    <span className="text-xs text-emerald-400 font-mono">ms (专线)</span>
                  </div>
                </div>
              </div>
              ...
              {/* Version Metric */}
              <div className="bg-[#171c26] border border-slate-700/80 p-3.5 flex items-center gap-3">
                <div className="p-2.5 bg-cyan-950/60 border border-cyan-800 text-cyan-400">
                  <Cpu className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-[11px] text-slate-400 font-mono">服务端架构 / 核心</div>
                  <div className="text-sm font-bold font-code text-cyan-300">
              ...
              <div className="bg-[#171c26] border border-slate-700/80 p-3.5 flex items-center gap-3">
                <div className="p-2.5 bg-amber-950/60 border border-amber-800 text-amber-400">
                  <Shield className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-[11px] text-slate-400 font-mono">服务器运行规则</div>
            ...
            <div className="bg-[#171c26] border border-slate-700/80 p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="text-xs font-code">
                  <span className="text-cyan-300 font-bold text-base">{onlineCount}</span>
                  <span className="text-slate-500"> / </span>
                  <span className="text-slate-400">{maxPlayers} MAX</span>
                </div>
              </div>

              {/* Progress Bar (Minecraft XP Bar Look) */}
              <div className="relative w-full h-3 bg-black border border-slate-700 overflow-hidden">
                <div
              ...
              <div className="pt-2">
                {data?.players.list && data.players.list.length > 0 ? (
                  <div className="flex flex-wrap gap-2 items-center">
                    <span className="text-[11px] text-slate-400 mr-2 font-mono">当前在线玩家:</span>
                    {data.players.list.map((player, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-1.5 px-2 py-1 bg-slate-900 border border-slate-700 text-xs font-code text-cyan-300"
                        title={player.name}
                      >
                        <img
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400 bg-black/40 px-3 py-2 border border-slate-800">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 text-center text-xs">
              <div className="p-2 bg-slate-800/40 border border-slate-700/60 font-mono text-slate-300">
                🛡️ 领地箱子防熊
              </div>
              <div className="p-2 bg-slate-800/40 border border-slate-700/60 font-mono text-slate-300">
                ⚡ Carpet 假人挂机
          ...
          {/* Card Bottom Actions */}
          <div className="bg-[#161c28] p-4 border-t border-slate-700/80 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs text-slate-400 font-mono">
              <span className="text-cyan-400">直连地址:</span>
              <span className="text-white font-code bg-black/70 px-2 py-1 border border-slate-700 select-all">
                {serverAddress}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <button
                onClick={handleCopy}
                className="flex-1 sm:flex-none mc-button text-xs py-2 px-4 flex items-center justify-center gap-1.5 bg-[#2a3447]"
              >
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copied ? '已复制 IP' : '复制服务器地址'}</span>
              </button>
